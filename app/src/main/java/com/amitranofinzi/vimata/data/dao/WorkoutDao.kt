...
//    suspend fun getWhereIn(field: String, values: List<String>): List<Workout>
    ...
    /**
     * Retrieves a workout from the database by its unique ID.
     *
     * @param id The unique ID of the workout to be fetched.
     * @return The Workout object with the specified ID, or null if not found.
     */
    @Query("SELECT * FROM workouts WHERE id = :id")
    suspend fun getWorkoutById(id: String): Workout?

    /**
     * Retrieves all workouts associated with a specific trainer.
     *
     * @param trainerID The ID of the trainer whose workouts are to be fetched.
     * @return A list of Workout objects created by the specified trainer.
     */
    @Query("SELECT * FROM workouts WHERE trainerID = :trainerID")
    suspend fun getWorkoutsByTrainer(trainerID: String): List<Workout>

    /**
     * Retrieves all workouts associated with a specific athlete.
     *
     * @param athleteID The ID of the athlete whose workouts are to be fetched.
     * @return A list of Workout objects associated with the specified athlete.
     */
    @Query("SELECT * FROM workouts WHERE athleteID = :athleteID")
